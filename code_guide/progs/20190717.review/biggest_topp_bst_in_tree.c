...
int biggest_topo_bst_in_tree(treenode *root)
{
    if (!root) 
        return 0;
    ...
    m = max(biggest_topo_bst_in_tree(root->left), m);
    m = max(biggest_topo_bst_in_tree(root->right), m);
    return m;
}
