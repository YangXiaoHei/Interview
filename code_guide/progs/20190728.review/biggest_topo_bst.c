...
int is_bstnode(treenode *root, treenode *n)
{
    if (!root)
        return 0;
    ...
    return is_bstnode(n->val < root->val ? root->left : root->right, n);
}

int max_topo(treenode *root, treenode *n)
{
    if (root && n && is_bstnode(root, n)) {
        return max_topo(root, n->left) + max_topo(root, n->right) + 1;
    }
    return 0;
}

int biggest_topo(treenode *root)
{
    if (!root)
        return 0;
    ...
    m = max(biggest_topo(root->left), m);
    m = max(biggest_topo(root->right), m);
    return m;
}
